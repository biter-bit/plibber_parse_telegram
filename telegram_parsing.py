...
async def download_image(message: dict, checksum_name_file: str, client, s3) -> str:
    ...
    file_path = f'photo/{checksum_name_file}.jpg'
...
async def get_messages(channel: int, client: object) -> list:
...
async def add_data_messages(channel: int, client, db, s3):
...
async def main(num_worker, client, db, s3):
    """Создаем ивент-луп парсинга"""
    start_num_channel = int(os.environ.get('START_NUM_CHANNEL'))
    finish_num_channel = int(os.getenv('FINISH_NUM_CHANNEL'))
    count_process = int(os.getenv('COUNT_PROCESS'))
    count_workers = int(os.getenv('COUNT_WORKERS'))
    result_list = get_list_channels(
        start_num_channel,
        finish_num_channel,
        count_workers,
        count_process,
    ...
    for id_idx in result_list:
        await get_channel(id_idx, client, db, s3)
        await add_data_messages(id_idx, client, db, s3)
